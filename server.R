...
shinyServer(function(input, output, session) {
  ...
  output$deg.gene.select <- renderUI({
    if(is.null(v$scData1) || is.null(v$isDEGdone)){
      plotly_empty()
    }else{
  ...
  output$Deg3.plot <- renderPlot({
    if(is.null(v$ips.markers) || is.null(v$isDEGdone)){
      ...
      withProgress(message="Generating DEG Plot...", value=0, {
        ...
        DoHeatmap(v$scData1, features = top10$gene, size = 5, angle = 45) + theme(axis.text.y = element_text(size = 3)) + scale_fill_gradientn(colors = c("blue", "white", "red")) + NoLegend()
      })
    }
  })
  ...
  output$cc.table <- DT::renderDataTable(
    v$means, options = list(scrollX = TRUE, scrollY = "400px"), server = FALSE)
  ...
  observeEvent(input$findVarGenes_bef_intg, {
    ...
    output$deg.gene.select_intg <- renderUI({
      if(is.null(v$ips.markers_intg)){
        ...
        selectInput("deg.gene_intg", label = "Gene to visualise",
                    choices = unique(v$ips.markers_intg$gene))
      }
    })
    ...
    output$Deg2.plot_intg <- renderPlot({
      if(is.null(v$ips.markers_intg)){
        ...
        withProgress(message="Generating DEG Plot...", value=0, {
          ...
          if(input$deg1b == "seurat_clusters"){
            v$scData1.combined$seurat_clusters -> Idents(v$scData1.combined)
            RidgePlot(v$scData1.combined, features = input$deg.gene_intg, group.by = "seurat_clusters")
          }
          else if(input$deg1b == "Predicted celltype"){
            v$scData1.combined$primary.predict -> Idents(v$scData1.combined)
            RidgePlot(v$scData1.combined, features = input$deg.gene_intg, group.by = "primary.predict")
          }
          else if(input$deg1b == "Metadata celltype"){
            v$scData1.combined$celltype -> Idents(v$scData1.combined)
            RidgePlot(v$scData1.combined, features = input$deg.gene_intg, group.by = "celltype")
          }
        })
      }
    })
    ...
    observeEvent(input$doCC1, {
      ...
      if (is.null(tpmFiles1)){
        ...
        withProgress(message = "Running cell-cell communication...", value = 0.3, {
          v$cellphone1 <- cellphone_for_seurat(v$scData1.combined)
          v$pvals1 <- read.delim("out/pvalues.txt", check.names = FALSE)
          v$means1 <- read.delim("out/means.txt", check.names = FALSE)
          v$decon1 <- read.delim("out/deconvoluted.txt", check.names = FALSE)
          print(v$pvals1)
          print(v$means1)
    ...
    output$CC.gene.select2 <- renderUI({
      if(is.null(v$scData1.combined)){
        return(NULL)
      }else{
        selectInput("CC.gene1b", label = "Gene",
                    choices = rownames(v$scData1.combined@assays$integrated@data), selected = rownames(v$scData1.combined@assays$integrated@data)[1], multiple = T, selectize = T)
      }
    })
    ...
    output$CC_plot2a <- renderPlot({
      if(is.null(v$isCC1done)){
        ...
        withProgress(message="Generating cell-cell communication plot...", value=0, {
          plot_cpdb(
            scdata=as.SingleCellExperiment(v$scData1.combined),
            cell_type1=input$CC.gene1a,
            cell_type2=input$CC.gene2a,  # this means all cell-types
            idents="primary.predict",
            means=v$means1,
            pvals=v$pvals1,
            genes=input$CC.gene1b
          )
                    #plot_cpdb3(cell_type1 = input$CC.gene1a, cell_type2 = input$CC.gene2a,
                    # scdata = as.SingleCellExperiment(v$scData1.combined),
                    # idents = 'primary.predict',
                    # means = v$means1,
                    # pvals = v$pvals1,
                    # deconvoluted = v$decon1, # new options from here on specific to plot_cpdb3
                    # keep_significant_only = TRUE,
                    # standard_scale = TRUE,
                    # remove_self = TRUE)
        })
    ...
    output$cc.table1a <- DT::renderDataTable(
      v$means1, options = list(scrollX = TRUE, scrollY = "400px"), server = FALSE)
    
    output$cc.table1b <- DT::renderDataTable(
      v$pvals1, options = list(scrollX = TRUE, scrollY = "400px"), server = FALSE)
    ...
    output$PCAplot_scvi_tpm1 <- renderPlotly({
      if(is.null(v$scData2) || is.null(v$isPCAdone_intg_scvi)){
        ...
        withProgress(message="Generating PCA Plot of integrated dataset...", value=0, {
          DimPlot(v$scData2, reduction = "scvi", label = T)
        })
      }
    })
    ...
    output$PCAplot_scvi_tpm2 <- renderPlotly({
      if(is.null(v$scData2) || is.null(v$isPCAdone_intg_scvi)){
        ...
        withProgress(message="Generating PCA Plot of integrated dataset...", value=0, {
          DimPlot(v$scData2, reduction = "scvi", label = T, group.by = 'batch', label.size = 3)
        })
      }
    })
    ...
    output$PCAplot_scvi_tpm3 <- renderPlotly({
      if(is.null(v$scData2) || is.null(v$isPCAdone_intg_scvi)){
        ...
        withProgress(message="Generating PCA Plot of integrated dataset...", value=0, {
          DimPlot(v$scData2, reduction = "scvi", label = T, group.by = 'celltype', label.size = 3)
        })
      }
    })
    ...
    output$vizPlot_intg_scvi <- renderPlot({
      if(is.null(v$scData2)){
        ...
        VizDimLoadings(v$scData2, dims = as.numeric(input$select.pc_intg_scvi), reduction = "scvi")
      }
    })
    ...
    output$PCHeatmap_intg_scvi <- renderPlot({
      if(is.null(v$scData2)){
        ...
        DimHeatmap(v$scData2, dims = as.numeric(input$select.pc_intg_scvi), reduction = "scvi")
      }
    })
    ...
    observeEvent(input$doDeg_intg_scvi, {
      ...
      if (is.null(tpmFiles1)){
        ...
        withProgress(message="Finding DEGs...", value=0, {
          v$scData2$primary.predict -> Idents(v$scData2)
      ...
      if (is.null(tpmFiles1)){
        ...
        withProgress(message="Generating heatmap...", value=0, {
          if(input$cell1a == "primary.predict"){
            ...
            v$cor1 <- cor(v$scData1.combined.rna.data.average1, method = "pearson")
            print(v$cor1)
      ...
      if (is.null(tpmFiles2)){
        ...
        withProgress(message="Generating heatmap...", value=0, {
          if(input$cell_m1 == "primary.predict"){
            ...
            v$cor_mult <- cor(v$scDatat.rna.data.average1, method = "pearson")
            print(v$cor_mult)
    ...
    output$Deg_mult1.plot <- renderPlotly({
      if(is.null(v$scDatat)){
        return(NULL)
      }else{
        withProgress(message="Generating DEG Plot...", value=0, {
          if(input$deg_mult2 == "seurat_clusters"){
            v$scDatat$seurat_clusters -> Idents(v$scDatat)
            FeaturePlot(v$scDatat, input$deg_mult.gene)
          }
          else if(input$deg_mult2 == "primary.predict"){
            v$scDatat$primary.predict -> Idents(v$scDatat)
            FeaturePlot(v$scDatat, input$deg_mult.gene)
    ...
    output$vis1.plot_multiome <- renderPlotly({
          ...
          if(input$scAnalysis_sp1 == "GraphST"){
            ...
            device = torch$device('cuda:0')
              ...
              withProgress(message="Clustering data...", value=0, {
                model = gt$GraphST$GraphST(adata = v$scData_spatial_graphst, device = device)
      ...
      observeEvent(input$doDeconv_graphst, {
        ...
        withProgress(message = "Performing deconvolution...", value = 0,{
          ...
          model = gt$GraphST$GraphST(v$test[0], v$test[1], device=device, deconvolution = T)
    ...
    observeEvent(input$doDeg_spatial, {
      ...
      observeEvent(input$calc_gene_activity_atac, {
        ...
        if (is.null(tpmFiles_atac)){
          ...
          withProgress(message="Calculating gene activity...", value=0, {
            gene.activities <- GeneActivity(v$atacData)
      ...
      output$gene_activity.select <- renderUI({
        if(is.null(v$atacData) || is.null(v$isGeneActivitydone)){
          ...
          selectInput("gene_activity", label = "Gene activity to visualise",
                      choices = rownames(v$gene.activities))
        }
      })
      
      observeEvent(input$load_eg_scRNA, {
        withProgress(message="Loading example reference scRNA-seq data...", value=0.5, {
          tpmFiles_ex_scRNA <- read.table('scRNA/pbmc.txt', header = T, row.names = 1, check.names = F)
      ...
      output$cellanno1_atac1.plot <- renderPlot({
        if(is.null(v$atacData) || is.null(v$isCELLAtacdone)){
          plotly_empty()
        }else{
          withProgress(message="Generating DEG Plot...", value=0, {
      ...
      output$peak_gene.atac.select <- renderUI({
        if(is.null(v$atacData)){
          ...
          selectInput("gene_peak", label = "Genes to visualise",
                      choices = rownames(v$atacData@assays$RNA@counts), multiple = T, selected = rownames(v$atacData@assays$RNA@counts)[7])
        }
      })
